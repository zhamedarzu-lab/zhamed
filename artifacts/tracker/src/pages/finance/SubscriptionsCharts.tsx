...
type HistoryRow = { month: string; total: number; [sub: string]: number | string };
...
export default function SubscriptionsCharts({ budget, colors }: Props) {
  const { data, loading } = useApi<History>("/api/finance/subscriptions/history");
  ...
  return (
    <Panel title="Breakdown by subscription" style={{ marginTop: "1rem" }}>
